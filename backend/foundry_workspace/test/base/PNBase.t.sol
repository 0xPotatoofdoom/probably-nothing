...
abstract contract PNBase is BaseTest {
    ...
    // FEE is set per-hook by the harness (3000 for static-fee hooks, 0x800000 for dynamic-fee hooks).
    uint24 internal constant FEE = HookConfig.FEE;
    ...
    function setUp() public virtual {
        ...
        // ctorArgs is generated by the Probably Nothing harness based on the hook's
        // detected constructor signature. We always pass address(this) as the second
        // arg so OZ v5 Ownable and other address-typed deps get a non-zero stub that
        // avoids zero-address reverts during hook deployment.
        bytes memory ctorArgs = HookConfig.ctorArgs(address(poolManager), address(this));
        ...
        // Uses a low-level self-call so hooks that gate beforeAddLiquidity (e.g. requiring
        // initialization before any LP) don't blow up setUp() — we just skip seeding
        // gracefully and let those tests fail on their own terms.
        ...
        address(this).call(
            abi.encodeCall(this._attemptSeedLiquidity, (amount0, amount1))
        );
        // If the call reverted, seedTokenId stays 0.
    }

    /// @dev Called via low-level self-call in setUp so a revert doesn't abort setUp.
    function _attemptSeedLiquidity(uint256 amount0, uint256 amount1) external {
